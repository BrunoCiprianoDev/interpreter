    ...
    public String interpretar() {
        ...
        while (matcher.find()) {
            ...
            if (match.startsWith("*") && match.endsWith("*")) {
                pilha.push(new Negrito(new Texto(match.substring(1, match.length() - 1))));
            } else if (match.startsWith("_") && match.endsWith("_")) {
                pilha.push(new Italico(new Texto(match.substring(1, match.length() - 1))));
            } else if (match.startsWith("~") && match.endsWith("~")) {
                pilha.push(new Sublinhado(new Texto(match.substring(1, match.length() - 1))));
            } else {
                pilha.push(new Texto(match));
            }
        }
